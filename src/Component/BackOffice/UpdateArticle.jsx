...
function UpdateAricle() {
  ...
  const [articleNameCat, setArticleNameCat] = useState(id);
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        setArticleNameCat(response.data.article.category.id);
        setId(response.data.article.category.id);
  ...
  const submitFormDataCat = async (values) => {
    ...
    let config = {
      ...
      url: `http://localhost:8000/api/category/crup/${idartcl}`,
      headers: {
  ...
  const submitFormData = async (values) => {
    ...
    let config = {
      method: "post",
      maxBodyLength: Infinity,
      url: `http://localhost:8000/api/article/crup/${idartcl}`,
      headers: {
  ...
  // };
  useEffect(() => {
    // Assuming article.category_id contains the category id of the article
    setArticleNameCat(article.category_id);
  }, [article.category_id]);
  return (
    <>
      <Box>
                ...
                >
                  ...
                  >
                    <TextField
                      ...
                      label="Prix unique"
                      InputLabelProps={{ shrink: true }}
                      //   defaultValue={article.unit_price}
